...
void timeformat(grp* group) {
    ...
    switch(group->tmfmt){
      case Epoch:
        ...
        regtxt = replace(regtxt, (char*)"$E", (char*)POSITIVE_INT_REGEX);
        regtxt = replace(regtxt, (char*)"$m", (char*)POSITIVE_INT_REGEX);
        break;
      case Date:
        ...
        regtxt = replace(regtxt, (char*)"$Y", (char*)POSITIVE_INT_REGEX);
        regtxt = replace(regtxt, (char*)"$o", (char*)POSITIVE_INT_REGEX);
        regtxt = replace(regtxt, (char*)"$D", (char*)POSITIVE_INT_REGEX);
        break;
      case Time:
        ...
        regtxt = replace(regtxt, (char*)"$H", (char*)POSITIVE_INT_REGEX);
        regtxt = replace(regtxt, (char*)"$M", (char*)POSITIVE_INT_REGEX);
        regtxt = replace(regtxt, (char*)"$S", (char*)POSITIVE_INT_REGEX);
        regtxt = replace(regtxt, (char*)"$m", (char*)POSITIVE_INT_REGEX);
        break;
      case DateTime:
        regorder[0] = strstr(text, "$Y");
...
void dataformat(grp* group) {
    ...
    switch(group->dtfmt) {
      case Int:
        group->dtordr[0] = 0;
        regtxt = replace(regtxt, (char*)"$I", (char*)INT_REGEX);
        break;
      case Float:
        group->dtordr[0] = 0;
        regtxt = replace(regtxt, (char*)"$F", (char*)FLOAT_REGEX);
        break;
      case String:
        group->dtordr[0] = 0;
        regtxt = replace(regtxt, (char*)"$S", (char*)STRING_REGEX);
        break;
      case Vec2d:
        ...
        regtxt = replace(regtxt, (char*)"$X", (char*)FLOAT_REGEX);
        regtxt = replace(regtxt, (char*)"$Y", (char*)FLOAT_REGEX);
        break;
      case Vec3d:;//empty statement to avoid compile errors
        ...
        regtxt = replace(regtxt, (char*)"$X", (char*)FLOAT_REGEX);
        regtxt = replace(regtxt, (char*)"$Y", (char*)FLOAT_REGEX);
        regtxt = replace(regtxt, (char*)"$Z", (char*)FLOAT_REGEX);
        break;
      case Vec4d:
        regorder[0] = strstr(text, "$X");
        regorder[1] = strstr(text, "$Y");
        ...
        regtxt = replace(regtxt, (char*)"$Y", (char*)FLOAT_REGEX);
        regtxt = replace(regtxt, (char*)"$Z", (char*)FLOAT_REGEX);
        regtxt = replace(regtxt, (char*)"$W", (char*)FLOAT_REGEX);
        break;
      case ColorRGB:
        ...
        mrgsrt(regorder, group->dtordr, 4);
        // create the regex from the user input
        regtxt = replace(regtxt, (char*)"$R", (char*)POSITIVE_INT_REGEX);
        regtxt = replace(regtxt, (char*)"$G", (char*)POSITIVE_INT_REGEX);
        regtxt = replace(regtxt, (char*)"$B", (char*)POSITIVE_INT_REGEX);
        regtxt = replace(regtxt, (char*)"$A", (char*)POSITIVE_INT_REGEX);
        break;
      case ColorHex:
        regtxt = replace(regtxt, (char*)"$X", (char*)HEX_REGEX);
        break;
      default:
        break;
    }
...
enum datafmt strtodtft(char* str) {
    if (!strcmp(str, "Int")) {
        return Int;
...
grp* readconfig(char* filein, size_t *grpcnt) {
    ...
    while ((read = getline(&line, &len, file)) != -1) {
        ...
        switch(startchar){
          ...
          case 't':
            if (line[1] == 'm') {
                dtgrps[grpnum].tmfmt = strtotmft(rdline);
            } else {
          ...
          case 'd':
            if (line[1] == 't') {
                dtgrps[grpnum].dtfmt = strtodtft(rdline);
            } else {
    ...
    */
    ...
    for (int i = 0; i < lncount; i++) {
        ...
        for (int j = 0; j < grpcnt; j++) {
            ...
            if (reti) {
                pt point;
                regmatch_t* matchptr = (regmatch_t*) malloc(sizeof(regmatch_t)*8);
                ...
                free(matchptr);
                matchptr = (regmatch_t*) malloc(sizeof(regmatch_t)*5);
